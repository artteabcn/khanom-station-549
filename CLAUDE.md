# Khanom Station 549 — Project CLAUDE.md
...
- **Property**: Khanom Station 549 Bed & Breakfast
...
| Token              | Value     | Usage                                         |
| ------------------ | --------- | --------------------------------------------- |
| `brand-pink`       | `#0d6b6b` | Primary accent (matches logo bg), CTAs        |
| `brand-pink-light` | `#c0dede` | Backgrounds, badges                           |
| `brand-pink-dark`  | `#084848` | Hover states                                  |
| `brand-blush`      | `#f0fafa` | Soft section backgrounds                      |
| `brand-cream`      | `#f5faf8` | Page background                               |
| `brand-teal`       | `#c47f17` | Secondary accent + body text + section labels |
| `brand-teal-light` | `#f0ddb0` | Dividers, soft accents                        |
| `brand-teal-dark`  | `#805010` | Footer bg, dark accents                       |
| `brand-ink`        | `#c47f17` | Body text (NEVER pure black)                  |
| `brand-ink-soft`   | `#4a8080` | Secondary text                                |

Fonts: **Merriweather** (h3, refined serif) + **Nunito** (body, geometric sans) + **Pacifico** (.section-title and .hero-title — brushed script matching the logo wordmark). Loaded from Google Fonts in `[locale]/layout.tsx`.
...
- Image slots (logo, hero, room, gallery) stored in R2 `khanomstation549-media` + D1 `content_images`
...
2. Application domain: `khanom-station-549.com`, paths: `/content/*` and `/api/admin/*`
3. Policy: include — emails are one of (greg@arkadya.tech, bradley@arkadya.tech, khanomstation549.lamai@gmail.com)
...
6. Note your team domain → set as `CF_ACCESS_TEAM_DOMAIN` (just the subdomain, e.g. `khanomstation549`)

**R2 bucket setup (one-time):**

```
wrangler r2 bucket create khanomstation549-media
```

Make the bucket public via dashboard → R2 → khanomstation549-media → Settings → Public access (enable r2.dev URL or attach a custom domain). Copy the public URL into `MEDIA_PUBLIC_URL` env var.
