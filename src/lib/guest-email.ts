...
<body style="margin:0;padding:0;background-color:#f5faf8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#c47f17;">
<div style="display:none;font-size:1px;color:#f5faf8;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;">${escapeHtml(preheader)}</div>
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color:#f5faf8;padding:32px 16px;">
  <tr><td align="center">
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="560" style="background:#ffffff;border-radius:16px;overflow:hidden;max-width:560px;width:100%;box-shadow:0 1px 2px rgba(15,123,110,0.06);">
      <tr><td style="background:#0d6b6b;padding:36px 32px 28px;text-align:center;">
        <h1 style="margin:0;color:#ffffff;font-size:32px;font-weight:300;letter-spacing:1.5px;line-height:1;">Khanom Station 549</h1>
      ...
      <tr><td style="padding:36px 36px 0;">
        <h2 style="margin:0;font-family:'Merriweather',Georgia,serif;font-weight:500;font-size:28px;color:#c47f17;line-height:1.2;">${greeting}</h2>
        <p style="margin:16px 0 0;font-size:15px;line-height:24px;color:#4a8080;">${escapeHtml(copy.intro)}</p>
      </td></tr>
      <tr><td style="padding:32px 36px 0;">
        ${sectionLabel(copy.stayTitle)}
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="border:1px solid #c0dede;border-radius:12px;border-collapse:separate;">
      ...
      <tr><td style="padding:24px 36px 0;">
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background:#f0fafa;border-radius:12px;border-collapse:separate;">
        ...
        <p style="margin:12px 0 0;font-size:12px;line-height:18px;color:#4a8080;">${escapeHtml(copy.balanceNote)}</p>
        <p style="margin:8px 0 0;font-size:12px;line-height:18px;color:#084848;">${escapeHtml(copy.nonRefundableNote)}</p>
      </td></tr>
      <tr><td style="padding:32px 36px 0;">
        ${sectionLabel(copy.contactTitle)}
        <p style="margin:0;font-size:14px;line-height:22px;color:#4a8080;">${escapeHtml(contactBody)}</p>
      </td></tr>
      <tr><td style="padding:32px 36px 36px;">
        <p style="margin:0;font-family:'Merriweather',Georgia,serif;font-size:22px;color:#c47f17;">${escapeHtml(copy.signOff)}</p>
        <p style="margin:6px 0 0;font-size:14px;color:#c47f17;font-weight:600;">${escapeHtml(copy.houseName)}</p>
        <p style="margin:6px 0 0;font-size:12px;color:#4a8080;">${escapeHtml(copy.houseAddress)}</p>
      </td></tr>
    </table>
  </td></tr>
</table>
...
function sectionLabel(label: string): string {
  return `<p style="margin:0 0 12px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#084848;font-weight:600;">${escapeHtml(label)}</p>`;
}

function row(label: string, value: string, isLast: boolean = false): string {
  const border = isLast ? "" : "border-bottom:1px solid #f0fafa;";
  return `<tr><td style="padding:14px 18px;${border}font-size:13px;color:#4a8080;">${escapeHtml(label)}</td><td style="padding:14px 18px;${border}font-size:14px;color:#c47f17;text-align:right;font-weight:600;">${value}</td></tr>`;
}

function rowHighlight(label: string, value: string): string {
  return `<tr><td style="padding:14px 18px;border-bottom:1px solid #c0dede;font-size:13px;color:#c47f17;font-weight:600;">${escapeHtml(label)}</td><td style="padding:14px 18px;border-bottom:1px solid #c0dede;font-size:14px;color:#c47f17;text-align:right;font-weight:700;">${value}</td></tr>`;
}

function rowTotal(label: string, value: string): string {
  return `<tr><td style="padding:14px 18px;font-size:13px;color:#4a8080;">${escapeHtml(label)}</td><td style="padding:14px 18px;font-size:14px;color:#c47f17;text-align:right;font-weight:600;">${value}</td></tr>`;
}
