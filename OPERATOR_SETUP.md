# Operator Setup — Khanom Station 549 Template
...
3. **`memory/MEMORY.md`** — auto-memory index (e.g. "Khanom Station 549 deploys to Pages, not Workers").
...
```powershell
Invoke-WebRequest -Uri "https://khanom-station-549.com/api/health" -UseBasicParsing
# expect: {"ok":true}
```
...
```powershell
Invoke-WebRequest -Uri "https://khanomstation549.pages.dev/content" -UseBasicParsing -MaximumRedirection 0
...
node node_modules\wrangler\bin\wrangler.js pages deployment list --project-name khanomstation549
```
...
| Database content   | Cloudflare D1 (`khanomstation549` database)                  | Yes — single source     |
| Uploaded images    | Cloudflare R2 (`khanomstation549-media` bucket)              | Yes — single source     |
...
$env:CLOUDFLARE_ACCOUNT_ID="a28e85c8d0762c4391b0f1dde60b6e75"
node node_modules\wrangler\bin\wrangler.js d1 migrations apply khanomstation549 --remote
...
# Deploy
$env:CLOUDFLARE_ACCOUNT_ID="a28e85c8d0762c4391b0f1dde60b6e75"
node node_modules\wrangler\bin\wrangler.js pages deploy .open-next --project-name khanomstation549 --commit-dirty=true
...
node node_modules\wrangler\bin\wrangler.js r2 object list khanomstation549-media

# Show D1 row count for bookings (sanity check after a booking flow test)
node node_modules\wrangler\bin\wrangler.js d1 execute khanomstation549 --remote --command "SELECT COUNT(*) FROM bookings"
```
...
| Service                  | URL                                                                            |
| ------------------------ | ------------------------------------------------------------------------------ |
| Production site          | <https://khanom-station-549.com>                                                |
| Pages dashboard          | <https://dash.cloudflare.com/?to=/:account/pages/view/khanomstation549>               |
| D1 dashboard             | <https://dash.cloudflare.com/?to=/:account/workers/d1>                         |
| R2 dashboard             | <https://dash.cloudflare.com/?to=/:account/r2/default/buckets/khanomstation549-media> |
...
1. Add their email to the **`Editors` policy** in the CF Access "Khanom Station 549 CMS" app
   (Zero Trust → Access → Applications → Khanom Station 549 CMS → Edit → Policies).
...
- **D1:** no automatic backup. Schedule a weekly export:
  ```powershell
  $env:CLOUDFLARE_ACCOUNT_ID="a28e85c8d0762c4391b0f1dde60b6e75"
  node node_modules\wrangler\bin\wrangler.js d1 export khanomstation549 --remote --output=backups/khanomstation549-$(Get-Date -Format yyyy-MM-dd).sql
  ```
