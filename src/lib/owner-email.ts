...
export function bookingOwnerEmail(data: {
  ...
  const tableRows = rows
    .map(
      ([k, v]) =>
        `<tr><td style="padding:6px 12px 6px 0;color:#4a8080;font-size:13px;">${escapeHtml(k)}</td><td style="padding:6px 0;color:#c47f17;font-size:14px;">${escapeHtml(v)}</td></tr>`
    )
    .join("");

  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:560px;">
      <h2 style="color:#0d6b6b;margin:0 0 16px;">New booking — Khanom Station 549</h2>
...
export function contactOwnerEmail(data: { name: string; email: string; message: string }): {
  ...
  return {
    ...
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:560px;">
        <h2 style="color:#0d6b6b;margin:0 0 16px;">New contact message — Khanom Station 549</h2>
        <p style="color:#c47f17;font-size:14px;"><strong>${escapeHtml(data.name)}</strong> &lt;${escapeHtml(data.email)}&gt;</p>
        <p style="color:#c47f17;font-size:14px;white-space:pre-wrap;">${escapeHtml(data.message)}</p>
      </div>
    `,
    replyTo: data.email,
  };
